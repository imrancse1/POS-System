                        ...
                        <div class="row">
                            <div class="col-md-12">
                                <table id="employeeDataTable" class="table table-bordered table-striped">
                                    ...
                                    <tbody>
                                        @if(count($personalInforations) > 0) @foreach($personalInforations as $key=> $personalInforation)
                                        <tr>
                                            <td>{{$personalInforation->personal_info_id}}</td>

                                            <td>{{$personalInforation->name}}</td>
                                            <td>{{$personalInforation->mobile_number}}</td>

                                            <td>{{$personalInforation->education_qualification}}</td>
                                            <td>{{$personalInforation->religion}}</td>
                                            <td>{{$personalInforation->dob}}</td>
                                            <td>{{Helper::maritalStatus($personalInforation->marital_status)}}</td>
                                            <td>{{$personalInforation->jobInformation->job_designation}}</td>
                                            <td>
                                                <img src="{{URL::to('public/images/personal_image/')}}/{{$personalInforation->image_path}}" height="40px" width="40px" alt="image">
                                            </td>

                                            <td>
                                                <label class="label label-warning" style="@if($personalInforation->status==1) display: none; @endif">
                                                    Inactive
                                                </label>
                                                <label class="label label-success" style="@if($personalInforation->status==0) display: none; @endif">
                                                    active
                                                </label>
                                            </td>
                                            <td>
...
<script type="text/javascript">
                ...
                success: function(data) {
                    ...
                    if (data.length > 0) {
                        $.each(data, function(index, value) {
                            ...
                            output += '<td>';
                            if (value.status == 1) {
                                output += '<a href="javascript:;" class="btn btn-warning btn-xs inactive_btn" id="reference_' + value.personal_info_id + '" style="display:show;" onclick="">' +
                                    '<i class="fa fa-ban inactiveTitle" title="Inactive"></i>' +
                                    '</a>&nbsp';
                            }
                            if (value.status == 0) {
                                output += '<a href="javascript:;" class="btn btn-success btn-xs active_btn" id="reference_' + value.personal_info_id + '" style="display:show;" onclick="">' +
                                    '<i class="fa fa-check-square-o" title="Active"></i>' +
                                    '</a>&nbsp';
                            }
                            output += '<a href="javascript:;" class="btn btn-danger btn-xs delete_btn" id="reference_' + value.personal_info_id + '" style="display:show;" onclick="">' +
                                '<i class="fa fa-trash" title="delete"></i>' +
                                '</a>&nbsp';
    ...
    $(document).on('click', '.inactive_btn', function() {
        ...
        var inactiveId = arr[1];
        // alert(reference)
        // return false;

        $.ajax({
            url: "inactivePersonalInfo/" + inactiveId,
            type: "GET",
            dataType: 'json',
            success: function(data) {
                //console.log(data);
                if (data.success == true) {
                    $('#statusTitleActive_'+inactiveId).html("Inactive").addClass('btn-warning');
    ...
    $(document).on('click','.active_btn',function(){
      ...
      $.ajax({
         url : 'activePersonalInfo/' + activeId,
         type: 'GET',
         dataType: "json",
         success: function(data){
            //console.log(data);
            if(data.success == true){
               $("#statusTitleInactive_"+activeId).html('Active').addClass('btn-success');
               $('.box-body-second').show();
               $(".messageBodySuccess").slideDown(1000).delay(3000).slideUp(1000).children().next().html(data.message);
            }else {
               $('.box-body-second').show();
    ...
    $(document).on('click', '.delete_btn', function() {
        ...
        if (confirmDlt) {
            $.ajax({
                url: "deletePersonalInfo/" + deleteId,
                type: "GET",
                dataType: 'json',
                success: function(data) {
                  //console.log(data);
                    if (data.success == true) {
                        $("#reference_" + deleteId).parent().parent().hide(1000).remove();
                        $('.box-body-second').show();
                        $(".messageBodySuccess").slideDown(1000).delay(3000).slideUp(1000).children().next().html(data.message);
                    } else {
                        $('.box-body-second').show();
